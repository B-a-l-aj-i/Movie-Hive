...
function NaviHeader() {
  ...
  const [url, seturl] = useState('https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=');
  ...
  function handleNowPlaying(){
      ...
      window.scrollTo({ top: 400, behavior: 'smooth' });
        seturl('https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=');
        setResetPage(true);
  ...
  function handleFavourite(){
    ...
    useEffect(() => {
        ...
        const storedFavMovies = keys.map((key) => {
          try {
...
>
  <ul className="flex gap-2 justify-evenly items-center cursor-pointer text-center">
    ...
    {fav && <p className='text-white p-6 bg-gray-900 font-bold' >Favourites 💖</p>}

        {
        <div className="bg-gray-900 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8 p-4">

        {fav && favMovies.map((movie) =><MovieCard key={movie.id} movie={movie}/>  )}
        {fav && favMovies.length==0 && <p className='text-white font-semibold' >No Favourites</p> }
    ...
    {fav && <hr></hr>}
     ...
     <Movies url={url} resetPage={resetPage} fave={fav} onResetComplete={() => setResetPage(false)} />
